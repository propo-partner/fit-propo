...
export const getStaticPaths = async () => {
  const data = ['s1u9swliszzgo7vg','b75r3in3q9x3j97p']
  //ここでデータをフェッチして、ルーティングのパスにしたい内容（ID）などを格納
  const paths = data.map((path) => ({
    params: {
      id: path
    }//paramsにオブジェクトのカタチでパスを格納していく。
  }))  
...
export default function EpisodePlay () {
  ...
  //   if (res.ok) {
  //     router.push("/episode/{id}/unlike")
  //   }
  // }
  
  return (
    <>
      <Link href="/episode/[id]/unlike" as={`/episode/${id}/unlike`}>
        <a>いまいちページへ</a>
      </Link>
